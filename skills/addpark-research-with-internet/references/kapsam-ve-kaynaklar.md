...
| Kendi genel bilgim (hatırdan mevzuat/standart metni) | Belge de değil, kaynak da değil — internetten birincil kaynaktan teyit edilir veya yazılmaz |

**Kontrol yöntemi:** her arama sonucunun `webUrl` alanına bak.
`/sites/BTech-ADD/` geçmiyorsa sonucu ele.

**Bu sürümde internet kapsam içidir, ancak diğer SharePoint siteleri ve
OneDrive kapsam DIŞI kalmaya devam eder.** İnternet izni, iç kaynak
kapsamını genişletmez.

Kullanıcı "diğer siteye de bak" derse: bunun skill'in dışına çıkmak
olduğunu tek cümleyle söyle, sonra istediğini yap. Sessizce kapsam
genişletme.

## 2b. Dış kaynak kalite eşiği

Kullanılabilir: mevzuat yayıncısı (EUR-Lex, Resmî Gazete), standart
kuruluşu (ISO, SAE, ASTM), üreticinin kendi sitesi, resmî kurum
duyurusu (TİTCK, TÜBİTAK, KOSGEB, Komisyon), hakemli yayın.

Kullanılmaz: blog, forum, içerik çiftliği, tarihsiz sayfa, yapay zekâ
üretimi özet siteleri, ikinci elden aktaran haber siteleri (birincil
kaynağa in).

Standart metni paralı duvarın arkasındaysa: bunu **söyle**, hatırdan
madde metni yazma. `[DIŞ — DOĞRULANAMADI]` etiketi bunun içindir.
